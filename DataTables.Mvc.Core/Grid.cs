...
namespace DataTables.Mvc.Core
{
    public class Grid : IHtmlString
    {
        ...
        private List<KeyValuePair<int, SortOrder>> _sorting;
        ...
        public string RenderJavascript()
        {
            ...
            if (_serverSide.HasValue)
                dataTable.AppendLine(String.Format("\"bServerSide\": {0},", _serverSide));
            ...
            if (_paginate.HasValue)
                dataTable.AppendLine(String.Format("\"bPaginate\": {0},", _paginate));

            if (_lengthChange.HasValue)
                dataTable.AppendLine(String.Format("\"bLengthChange\": {0},", _lengthChange));

            if (_pageLength > 0)
                dataTable.AppendLine(String.Format("\"iDisplayLength\": {0},", _pageLength));

            if (_filter.HasValue)
                dataTable.AppendLine(String.Format("\"bFilter\": {0},", _filter));

            if (_sort.HasValue)
                dataTable.AppendLine(String.Format("\"bSort\": {0},", _sort));

            if (_showInfo.HasValue)
                dataTable.AppendLine(String.Format("\"bInfo\": {0},", _showInfo));

            if (_autoWidth.HasValue)
                dataTable.AppendLine(String.Format("\"bAutoWidth\": {0},", _autoWidth));
            ...
            if (_saveState.HasValue)
                dataTable.AppendLine(String.Format("\"bStateSave\": {0},", _saveState));
